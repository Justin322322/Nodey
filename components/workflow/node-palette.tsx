...
export function NodePalette({ onNodeDrag }: NodePaletteProps) {
  ...
  const SectionHeader = ({ title }: { title: string }) => (
    <button className="w-full flex items-center justify-between text-sm font-medium text-gray-700 mb-2" onClick={() => toggle(title)}>
      <span>{title}</span>
      {open[title] ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}
    </button>
  )
  ...
  return (
    <div className="bg-white border-r border-gray-200 w-72 overflow-y-auto">
      <div className="p-4">
            ...
            <div className="space-y-2">
              {templates.map((t) => (
                <div key={t.key} className="p-3 border border-gray-200 rounded-md">
                  ...
                  <div className="mt-2">
                    <Button size="sm" onClick={() => insertTemplate(t.key)}>Insert</Button>
                  </div>
                </div>
              ))}
            </div>
