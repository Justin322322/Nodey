...
describe('HttpNode', () => {
  ...
  describe('executeHttpNode', () => {
    const createContext = (config: Partial<HttpNodeConfig>): NodeExecutionContext => ({
      nodeId: 'test-node',
  ...
  describe('HTTP_NODE_DEFINITION', () => {
    describe('validation', () => {
      ...
      it('should validate authentication value when auth type is not none', () => {
        ...
        expect(errors).toContain('Authentication value is required for selected auth type')
      })
    ...
    describe('schema properties', () => {
      ...
      it('should have descriptive label and description', () => {
        ...
        expect(HTTP_NODE_DEFINITION.description).toBe('Make an HTTP request to an external API')
      })
